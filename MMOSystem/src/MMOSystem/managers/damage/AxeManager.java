...
public class AxeManager extends Manager{

	private static AxeManager swordmanager;

	private static AxeSkillManager skillmanager;

	public final MMOSystem plugin;

	public static AxeManager getInstance(){
		return swordmanager;
	}
	...
	public String getDescription(){
		...
		d = d + "§6<主动技能>§7：§f" + AxeSkillManager.getSkillName() + "(潜行状态下使用斧触发)\n";
		d = d + "§6<经验计算>§7：§f使用斧造成伤害\n";
		d = d + "§6<等级作用>§7：§f减少主动cd，增强主动效果\n";
        //d = d + "§6<获得称号>§7：§fLv100§8->§f玩斧者 §fLv300§8->§3斧之精通 §fLv600§8->§3斧正天下 §fLv1000§8->§8获得§c▲§8标记";
        return d;
	}
	...
	public AxeManager(MMOSystem plugin){
		this.plugin = plugin;
		swordmanager = this;
		skillmanager = new AxeSkillManager(this);
	}
...
class AxeSkillManager extends SkillManager{

	public static String getSkillName(){
		return "暴躁老哥";
	}

	private AxeManager swordmanager;

	private HashMap<Player, Integer> using = new HashMap<>();   //默认0，启动时1，冷却中2，冷却结束0

	public AxeSkillManager(AxeManager manager){
	...
	public int getSkillCd(Player p){
		int level = swordmanager.plugin.getLevel(p, AxeManager.getJobName());
